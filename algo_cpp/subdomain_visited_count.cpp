...
class Solution {
public:
    void count_domain(string s, unordered_map<string, int> &mem) {
      int space = s.find(' ');
      int count = atoi(s.substr(0, space).c_str());
      mem[s.substr(space+1)] += count;
    ...
    vector<string> subdomainVisits(vector<string>& cpdomains) {
      ...
      for_each (mem.begin(), mem.end(), [&](auto it) {
...
int main() {
  ...
  vector<string> r = s.subdomainVisits(v);
  for_each(r.begin(), r.end(), [&](auto s) {cout << s << ',';});
  cout << endl;
  return 0;
}
